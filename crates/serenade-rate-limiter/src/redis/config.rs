...
/// Connection and key-prefix settings for [`super::RedisRateLimiterStorage`].
#[derive(Clone, Debug)]
pub struct RedisRateLimiterStorageConfig {
    /// Redis URL (`redis://…`).
    pub url: String,
    /// Key prefix (default `serenade:rate-limiter:`).
    pub prefix: String,
    /// r2d2 max pool size.
...
impl RedisRateLimiterStorageConfig {
    /// Builds a config for `url` with defaults.
    #[must_use]
    pub fn new(url: impl Into<String>) -> Self {
        Self {
            url: url.into(),
            prefix: "serenade:rate-limiter:".to_owned(),
            pool_max_size: 8,
